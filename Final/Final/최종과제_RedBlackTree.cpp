...
Node* findmin(Tree* RB,Node* x)
{
	while(x->left != RB->nil)
		x =x->left;
	return x;
}
Node* findmax(Tree* RB,Node* x)
{
...
Node * RBsuccessor(Tree* RB, Node* x)
{
	if(x->right !=RB->nil) 
		return findmin(RB,x->right);
	Node* y  = x->parent;
  ...
  while(succ != RB->nil && x  == succ->right)
  {
      x = succ;
      succ = succ->parent;
  }
...
Node * RBpredecessor(Tree* RB, Node* x)
{
	if(x->left !=RB->nil) 
		return findmax(RB,x->left);

	Node* y  = x->parent;

	while( y !=RB->nil && x==y->left)
	{
		x=y;
		y=y->parent;
	}
	...
	/*if(x->left==RB->nil)
	{
		if(x==x->parent->left)
		{
			while(x!=x->parent->right&&x!=RB->root)
			{
				x=x->parent;
			}
			if(x==RB->root)
...
Node* RBnearsearch(Tree* RB, Node* x, int data)
{
	while(x!=RB->nil)
		{
			if(x->key<data)
	...
	return RBpredecessor(RB,x);
}

void RBfinal(Tree* RB, FILE * fread,FILE* fout,int data) 
{
	Node* a;
	Node* b;
	Node* c;
	...
	if(data!=0)
		{
			a = RBsearch(RB,RB->root,data);	
		if(a==RB->nil)
		{
			RBinsert(RB,data);
			a = RBsearch(RB,RB->root,data);
			b=RBpredecessor(RB,a);
			c=RBsuccessor(RB,a);
			Rbdelete(RB,a);
			if(b==RB->nil)
			{
		...
		else 
		{
			...
			b=RBpredecessor(RB,RBsearch(RB,RB->root,data));
			c=RBsuccessor(RB,RBsearch(RB,RB->root,data));
			if(b==RB->nil){
   ...
   //     fclose(fout);
	...
	if(a==RB->nil)
	{
		a=RBnearsearch(RB,RB->root,data);
	...
	//   printf("filename : ");
	 //  scanf_s("%s",fname,sizeof(fname));
      ...
      do 
      {
		...
		fopen_s(&fp3,"output01.txt","w");
		fopen_s(&fp2,"search01.txt","r");
		while (inputdata!=0)
		{
			printf("\n");
			fscanf_s(fp2,"%d",&inputdata);
			RBfinal(test,fp2,fp3,inputdata);
			fprintf(fp3,"\n");
		}
        fclose(fp2);
		fclose(fp3);
